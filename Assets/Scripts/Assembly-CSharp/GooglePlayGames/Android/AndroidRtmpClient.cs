...
namespace GooglePlayGames.Android
{
	internal class AndroidRtmpClient : IRealTimeMultiplayerClient
	{
		...
		{
			Logger.d(string.Format("AndroidRtmpClient.SendMessage, reliable={0}, participantId={1}, data[]={2} bytes, offset={3}, length={4}", reliable, participantId, data.Length, offset, length));
		...
		private void UpdateRoom()
		{
			...
			if (mSelf == null)
			{
				...
				mSelf = new Participant("?", text3, Participant.ParticipantStatus.Unknown, new Player("?", text2), false);
			}
			list2.Sort();
			list3.Sort();
			string[] array;
