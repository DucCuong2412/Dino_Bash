...
public class tk2dTextMesh : MonoBehaviour, ISpriteCollectionForceBuild
{
	...
	private void FormatText(ref string _targetString, string _source)
	{
		...
		while (doBestFit)
		{
			doBestFit = data.lineCount > 0 && _source.Length > 0 && Application.isPlaying;
			...
			{
				lines_so_far++;
				if (doBestFit && (lines_so_far > data.lineCount || force))
				{
					float num7 = 0.05f;
					Vector3 vector = data.scale;
					vector.x -= num7;
					vector.y -= num7;
					vector.z -= num7;
					data.scale = vector;
			...
			for (int i = 0; i < _source.Length; i++)
			{
				...
				else if (num2 + tk2dFontChar2.p1.x * data.scale.x > num)
				{
					if (num3 > 0f)
					{
						num3 = 0f;
