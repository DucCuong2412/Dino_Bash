...
public static class Konfiguration
{
    ...
    public static int CoinBonusForTappingOnNotification
    {
        get
        {
            return 0;// GameConfig.CoinBonusForTappingOnNotification;
        }
    }
    ...
    public static string RateURL
    {
        get
        {
            return string.Empty;// GameConfig.googlePlayRateURL;
        }
    }
    ...
    public static LevelData GetLevelData(int levelID)
    {
        ...
        {
            Debug.LogError("Levels list is empty or not initialized.");
            return new LevelData(LevelTheme.map00, 0, new List<LevelEnemy>());
        }
        int index = Mathf.Clamp(levelID, 0, levels.Count - 1);
        if (index < 0 || index >= levels.Count || levels[index] == null)
        {
            Debug.LogError($"Requested level index {levelID} is out of range or null. Returning first level as fallback.");
            return levels[0];
        }
        return levels[1];
    }
