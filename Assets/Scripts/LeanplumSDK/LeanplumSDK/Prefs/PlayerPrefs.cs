...
namespace LeanplumSDK.Prefs
{
	internal static class PlayerPrefs
	{
		...
		private static int flushFrequencyInSeconds;
		...
		static PlayerPrefs()
		{
			...
			flushFrequencyInSeconds = 300;
			fileName = Application.persistentDataPath + "/LeanplumPrefs.txt";
		...
		private static void Serialize()
		{
			...
			{
				if (enumerator != null && enumerator.Key != null && enumerator.Value != null)
				{
		...
		private static void Deserialize()
		{
			...
			foreach (string text in array2)
			{
				...
				if (array3.Length >= 3)
				{
					string key = DeEscapeNonSeperators(array3[0]);
					playerPrefsHashtable[key] = GetTypeValue(array3[2], DeEscapeNonSeperators(array3[1]));
				}
				if (array3.Length != 3)
				{
